...
スループット容量でほぼ決まり、Multi-AZ（第一世代）・512 MB/s では**論理 14.0 TiB**、
Single-AZ（第一世代）・512 MB/s では**論理 6.5 TiB**でした。
...
| スループット容量 | Multi-AZ（第一世代） | Single-AZ（第一世代） |
|---|---|---|
...
| 次元 | 第一世代 | 第二世代 |
|---|---|---|
| スループット容量（Multi-AZ） | **$1.511**（`APN1-ThroughputCapacity.MAZ`） | **$3.148**（`APN1-ThroughputCapacity.MAZ2`） |
| スループット容量（Single-AZ） | **$0.906**（`APN1-ThroughputCapacity.SAZ_2N`） | **$2.013**（`APN1-ThroughputCapacity.SAZ_2N2`） |
| SSD ストレージ | $0.300 / $0.150 | **同額**（別 SKU、`.MAZ2:SSD` / `.SAZ_2N2:SSD`） |
| 超過 SSD IOPS | $0.0408 / $0.0204 | **同額**（別 SKU） |
| 容量プール・リクエスト | $0.0476 / $0.0238、$0.00037 / $0.0047 per 1,000 | **第二世代の usagetype が存在しません** |

**固定費の大半がスループット容量なので、第二世代では逆転点が遠のきます。** 本書と
...
### 可用性を揃えても容量で順位が反転すること

**上の 20 TiB は 1 点です。容量を下げると 99.99% を揃えたままでも EBS 2 AZ が安くなります。**
逆転点は必要スループット容量で動くので、**定数として引けません。**

| 必要スループット容量 / IOPS | 逆転点（論理） | 論理 100 GB での比較 |
|---|---|---|
| 512 MB/s / 5,000 | 8,523 GB（8.3 TiB） | EBS 2 AZ $80.86 対 FSx for ONTAP $1,159.49（**14.3 倍**） |
| 256 MB/s / 3,000 | 4,545 GB（4.4 TiB） | $32.28 対 $694.02（21.5 倍） |
| 128 MB/s / 3,000 | 2,556 GB（2.5 TiB） | $19.98 対 $500.61（25.1 倍） |

**逆転点は 2.5〜8.3 TiB の範囲にあります。** 下回る側では、FSx for ONTAP の最小 SSD 1,024 GiB と
スループット容量の床が支配します。

**下回る側で判断を決めているのは、ストレージの単価ではありません。** 上の EBS 2 AZ の額は
ストレージだけで、[数えていない 4 項目](#ebs-2-az-構成で数えていない費用)は容量に比例しません。
EC2 1 台・数十 GiB のために複製と切り替えの仕組みを自前で作って運用するなら、比較は
ストレージ対ストレージではなくなります。**「単一インスタンス専用のデータ領域だから EBS」という
判断は、AZ 障害をまたぐ要件が無い場合に限って成立します。**
...
- **データ転送**: AZ 間・リージョン間。Multi-AZ のノード間レプリケーションはスループット
  容量の料金に含まれます
- **移行ツール**: **AWS Transform の VMware 移行エージェントは無料**なので、除外している
  のは Shift Toolkit の費用（未確認）と、移行中に両側の容量を同時に持つ期間です。ATX が
  作らせたリソースの費用は別で、Finalize 後も残る NLB は未算出のままです
  （[GA 検証レポート](atx-fsxn-ga-verification.md) の 9.9 と U17）
