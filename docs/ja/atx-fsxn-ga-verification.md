...
**最終更新**: 2026-09-15（課金の所在を pricing ページと本アカウントの請求データで確認し 9.9 に追加。移行実行の実測値は 2026-09-04 のまま）
**ステータス**: スコープ確定から、初期化・証明書認証・設定保存・レプリケーション実行・テストインスタンス起動・データ整合性確認、そして **Finalize は承認を得て実行**（12.10）。撤去まで完了（13 章）
...
**入力検証の強さがコンソールと API で異なる。** 4.4 の通り API は存在しないシークレット ARN を受理するが、コンソールは 3 項目すべてを必須として拒否する。さらに「追加のセキュリティグループ」が**必須であること**は MGN ユーザーガイドから読み取れない。手順（Step 5 の項目 7）にセキュリティグループを選ぶ指示はあるが、未入力だと保存が拒否されるとは書かれておらず、必須性はコンソールで初めて判明する。
...
| Finalize 時の split の所要時間 | **実測済み**。約 3 分後に開始し 60 秒未満で完了（7.93 GiB、12.10） |
...
2026-09-04 時点で、以下を対象に最小 ONTAP バージョンの記載を探したが**見つからなかった**。2026-09-11 に FSx for ONTAP 構成ページと Troubleshooting ページを全文（それぞれ 20,994 字・12,676 字）読み直したが、依然として記載は無い。
...
### 9.9 課金の所在 [文書 + 実測 / 2026-09-15]

**ATX のジョブを放置しても課金されない。** ジョブは月単位で人の入力を待つ設計であり、待っている状態そのものに課金はない。費用が発生するのは ATX が作らせたリソース側である。

| 主張 | 区分 | 根拠 |
|---|---|---|
| VMware 移行エージェントの利用料は無料 | [文書] | [AWS Transform pricing](https://aws.amazon.com/transform/pricing/) のエージェント表が Assessment / Windows / Mainframe / VMware を Free、Custom transformation agent と continuous modernization を Paid と区分している |
| ATX 自体に追加料金はない | [文書] | [What is AWS Transform?](https://docs.aws.amazon.com/transform/latest/userguide/what-is-service.html) に "There are no additional charge to use AWS Transform" |
| 作られたリソースは通常料金で課金される | [文書] | 同 pricing の FAQ。**費用はここに集中する** |
| ジョブが月単位で待つのは設計 | [文書] | 同 User Guide の用語定義が Job を "A long-running process (weeks/months+)" としている |
| 有料の Custom agent でもアイドル時間は課金されない | [文書] | 同 pricing。課金単位は agent minute で、"You are only charged for active agent work during server-side operations, not for user idle time or client-side operations" |
| 本アカウントで 84 日放置して請求ゼロ | [実測] | 下記 |

**実測の内容（2026-09-15 実施）**: ジョブ `vmware-to-ec2-fsxn-poc` は 2026-06-23 にインベントリ提出待ちになってから **84 日**が経過し、`EXECUTING` のまま HITL タスク 0・コネクタ 0 である。Cost Explorer で 2026-06-01 → 2026-09-16 の `UnblendedCost` を月次・`SERVICE` 別に取得（`Credit` / `Refund` を除外）したところ、**4 期間すべてに `AWS Transform` のキーが存在しない。**

**キーの不在を「0 円の行」と混同しないための対照が同じデータの中にある。** 無料でも利用があればゼロ額の行は立つ。同じ期間で `AWS Application Migration Service` は 9 月に $0.00 で現れており（エンドツーエンド実行は 09-04）、`Amazon Q` も 7 月・8 月に $0 で現れている。**ATX はどの月にも行そのものが無い。** また ATX は 9 月にもコンソール手順で実際に操作しているので、この不在はアイドルであることだけの帰結でもない。

**この実測が言っていない範囲**:

- 1 アカウント・1 期間・**VMware 移行エージェント経路のみ**。有料の Custom transformation agent については上表の [文書] だけが根拠であり、実測していない
- **作られたリソースの費用については何も言っていない。** U17（Finalize 後も残る NLB の費用）は未算出のままである
- **リソース単位への遡及的な費用の割り当ては本アカウントでは取れない。** `getCostAndUsageWithResources` を `Amazon Elastic Load Balancing` に絞って 2026-09-02 → 2026-09-16 で日次取得すると `NoResourceId` しか返らず、月次合計（9 月 $31.32）と桁が合わない。リソースレベルのデータが有効になっていない。**リソース単位のデータは 14 日しか遡れない**ため、09-04 の測定について取れる期間は 09-18 頃に閉じる
...
| U2 | エンドツーエンドの移行実行 | **解消**（12 章） | テストインスタンス起動からカットオーバー、**Finalize まで完了**（12.10）。データ整合性も一致 |
...
| U5 | `lun move start` による移行後最適化の実挙動 | 未検証 | U2 は解消したが、移行後の `lun move` は本検証では未実行。移行直後は 1 サーバーの全 LUN が 1 ボリュームに載る（12.3）ため、復旧の粒度を分けたい場合に必要になる |
| U6 | 最小 ONTAP バージョン要件 | 未確認 | 公開ドキュメントに記載が見つからない（2026-09-04 調査、2026-09-11 に本文を通読して再確認） |
...
| U13 | 「追加のセキュリティグループ」が**必須である**ことの公式記述 | 未確認 | 2026-09-11 時点で手順（Step 5 の項目 7）にセキュリティグループを選ぶ指示はある。**未入力だと保存が拒否される**という記述は無く、必須性はコンソールの検証メッセージでのみ判明する（2026-09-04 調査、2026-09-11 再確認） |
...
| U21 | 割り当て不整合を MGN 側で再評価させる方法 | **解消**（2026-09-09） | `update-replication-configuration` では修復できない（矛盾する 3 種のエラー、`isBootDisk` は読み取り専用）。**正攻法はインストーラの再実行**で、[導入手順](https://docs.aws.amazon.com/mgn/latest/ug/linux-agent.html) の手順 6 が、複製対象の一覧が実機とずれた場合の修復手段として明記している。ソースサーバーの削除は不要。当方が実行した削除 + 再登録は、いちばん重い手段を選んでいた。ただし `stagingDiskType` まで再導出されるかは同記述からは決まらず未検証 |
| U17 | NLB の課金影響 | 未算出（**残存条件は判明。遡及算出の手段も塞がっている**） | NLB は**レプリケーション期間中だけでなく Finalize 後も残る**（12.10）。手作業の撤去が必要。公開ドキュメントに記述が見つからず、費用は未見積り。**本アカウントではリソース単位の費用データが有効でないため、撤去済みの NLB に遡って額を割り当てられない**（9.9）。算出するには別環境で、リソースレベルのデータを有効にしてから測る |
| U24 | ATX のジョブを放置した場合の課金 | **解消**（9.9） | VMware 移行エージェントは無料。**84 日放置したジョブで Cost Explorer に `AWS Transform` の行が立たない**ことを実測。費用は作られたリソース側に集中する |
| U23 | Finalize が NLB / VPC エンドポイントサービスを残すことの公式記述 | 未確認 | 実測では T0 + 36 分まで削除されなかった（12.10）。EBS は約 33 分で削除される。公開ドキュメントに記述が見つからない（2026-09-04 調査、2026-09-11 に本文を通読して再確認） |
...
| 公開ドキュメントの更新待ち | U6（最小 ONTAP バージョン）、U7（EVS の GA）、U13（追加セキュリティグループ必須化）、U22（再登録時のテンプレート再作成）、U23（Finalize が残すリソース） | 更新を待つか AWS / NetApp へ確認する。反映されるまで [未確認] のまま扱う |
| 別環境が必要 | U19（`SNAPSHOT_FAIL` 時に失われる差分の範囲）、U17（NLB の費用） | U19 は書き込みが継続するソースが必要。**U17 は既存の請求データからは取れない**（9.9）ため、リソースレベルの費用データを有効にした環境で測り直す |
...
**費用の含意**: レプリケーション期間中、**利用者のアカウントに NLB が常駐する**。NLB の時間課金と LCU 課金が発生する。参照した AWS ドキュメントとブログにこの記述は見つからなかった（2026-09-04 調査、2026-09-11 に本文を通読して再確認）。移行コストの見積りに含める必要がある。
...
第二に、**ターゲットボリュームはステージングボリュームの Snapshot に依存している**。起動直後の物理消費は差分のみで、両ボリュームの `space.used`（それぞれ約 7.9 GiB）は論理値である。この依存関係があるため、**Finalize がステージングボリュームを削除するには FlexClone をスプリットするか削除順序を制御する必要がある**。スプリットは約 7.93 GiB の書き込みを伴う。**この予測は 12.10 で実測と一致した**（U16 解消）。
...
**EBS ボリュームは削除される（約 33 分後）。NLB と VPC エンドポイントサービスは残る。** MGN が作った FSx プロキシは、移行完了後も課金され続けるため**手作業の撤去が必要である**。公開ドキュメントにこの残存についての記述は見つからなかった（2026-09-04 調査、2026-09-11 に本文を通読して再確認）。
...
*本レポートは 2026-09-04 時点の公開ドキュメントと、検証アカウント（ap-northeast-1）での実機確認に基づく。コンソールからの初期化と証明書認証、レプリケーションテンプレートへの `FSX_ONTAP` 設定保存、レプリケーション実行からカットオーバー、Finalize、撤去までを実測した（12 章・13 章）。残る未確認は 11 章にある。*
