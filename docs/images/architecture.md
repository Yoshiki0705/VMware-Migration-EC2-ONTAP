...
パス優先度の値（優先側 `prio=50` / 待機側 `prio=10`）は [FSx for ONTAP iSCSI 設定ガイド](../ja/fsxn-iscsi-setup.md) にあります。**パス本数を帯域から決める算術は、このリポジトリには置いていません。** 以前ここには「算術も設定ガイドにある」と書いてありましたが、ガイドには優先度の値しかありませんでした。除数に使う 1 クライアント 5 Gbps（約 625 MBps）は AWS の iSCSI / NVMe/TCP プロビジョニング手順に記載があります。**その値に対するブロックプロトコルの実測は、このプロジェクトでも姉妹リポジトリでも未了です。**

除数と実測のずれを測る計画が [ブロックプロトコルの測定計画](https://github.com/Yoshiki0705/S3-Burst-on-ONTAP-Files/blob/6b593148b634acbe1c57bc750f224d40956197f6/docs/ja/verification/block-protocol-matrix-plan.md) にあります。**リンク先は計画であって結果ではありません。** F-1（iSCSI）/ F-2（NVMe/TCP）/ F-3（キャッシュ常駐と非常駐）はいずれも未実施です。

計画のうち、こちらの読み方に効く点が 2 つあります。**`nr_sessions=1` でも iSCSI は両ノードへ 1 本ずつ計 2 本の TCP を張るので、素朴な「1 セッション」はファイル側の 1 接続と並べられません**（[「1 セッション」が単一フローではないこと](https://github.com/Yoshiki0705/S3-Burst-on-ONTAP-Files/blob/6b593148b634acbe1c57bc750f224d40956197f6/docs/ja/verification/block-protocol-matrix-plan.md#1-セッションが単一フローではないこと)）。そして測定は raw device に対して行うので、**LUN の上にファイルシステムを作るこちらの構成の実測値としては引用できません。**図に入れていないのは、**画像が縮小されたときに最初に読めなくなるのが図中で最も文字数の多い部分**だからです。
