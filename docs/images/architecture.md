...
パス優先度の値（優先側 `prio=50` / 待機側 `prio=10`）は [FSx for ONTAP iSCSI 設定ガイド](../ja/fsxn-iscsi-setup.md) にあります。**パス本数を帯域から決める算術は、このリポジトリには置いていません。** 以前ここには「算術も設定ガイドにある」と書いてありましたが、ガイドには優先度の値しかありませんでした。除数に使う 1 クライアント 5 Gbps（約 625 MBps）は AWS の iSCSI / NVMe/TCP プロビジョニング手順に記載がありますが、**その値に対するブロックプロトコルの実測はこのプロジェクトでは未了です。**図に入れていないのは、**画像が縮小されたときに最初に読めなくなるのが図中で最も文字数の多い部分**だからです。
